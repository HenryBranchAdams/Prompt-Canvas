...
MAX_DRIZZLE_MIGRATION_BYTES = 20_000
...
def validate_seed(records: list[dict[str, Any]], sql_outputs: list[str]) -> None:
...
def validate_staged_deployment(sql_output: str, drizzle_chunks: list[str]) -> None:
    if len(drizzle_chunks) < 2:
        raise SystemExit("Sites D1 deployment seed must separate staging from publication")
    database = sqlite3.connect(":memory:")
    try:
        database.executescript((ROOT / "migrations" / "0001_official_prompt_library.sql").read_text(encoding="utf-8"))
        database.executescript(sql_output)
        before = database.execute(
            "SELECT (SELECT count(*) FROM official_prompts WHERE active=1), "
            "(SELECT count(*) FROM official_prompt_versions), "
            "(SELECT count(*) FROM official_prompt_facets), "
            "(SELECT count(*) FROM official_prompt_fts), "
            "(SELECT build_hash FROM official_prompt_catalog_meta WHERE singleton=1)"
        ).fetchone()
        for chunk in drizzle_chunks[:-1]:
            database.executescript(chunk)
        ...
        staged = database.execute("SELECT count(*) FROM official_prompts_staging").fetchone()[0]
        if before != after or staged == 0:
            raise SystemExit("Partial Sites D1 seed chunks can alter the published catalog")
    finally:
        database.close()
...
def validate_search_evaluations(database: sqlite3.Connection) -> None:
    ...
    allowed_facets = {"intent", "input-mode", "subject-kind", "output-kind", "preservation-need", "collection", "category", "family", "capability"}
    for case in cases:
...
def seed_sql(records: list[dict[str, Any]], build_hash: str) -> str:
    ...
    lines.append(f"DELETE FROM official_prompt_facets WHERE prompt_id IN ({ids});")
    for record in records:
        ...
        lines.append("DELETE FROM official_prompt_fts WHERE prompt_id = " + sql(record["id"]) + ";")
    ...
    lines.append(f"UPDATE official_prompts SET active=0 WHERE id NOT IN ({ids});")
    lines.append(f"DELETE FROM official_prompt_fts WHERE prompt_id NOT IN ({ids});")
...
def split_drizzle_seed(sql_output: str) -> list[str]:
    statements = [line for line in sql_output.splitlines()
                  if line and not line.startswith("-- Generated") and line not in {"BEGIN TRANSACTION;", "COMMIT;"}]
    # Sites applies each Drizzle migration through its own storage transaction.
    # Explicit SQL transaction statements are rejected by the Durable Objects
    # storage API used by the deployment runner.
    prompt_columns = ("id, slug, current_version, title, short_description, user_promise, collection, category, "
                      "template_family, default_operation, input_mode, output_kind, complexity, required_input_summary, "
                      "preservation_summary, badges, aliases, search_text, thumbnail_path, thumbnail_alt, featured_rank, "
                      "active, created_at, updated_at")
    version_columns = ("prompt_id, version, template_json, template_schema, content_hash, normalized_prompt, negative_prompt, "
                       "source_json, change_note, created_at, published_at")
    staging_statements = [
        "CREATE TABLE IF NOT EXISTS official_prompts_staging AS SELECT * FROM official_prompts WHERE 0;",
        "CREATE TABLE IF NOT EXISTS official_prompt_versions_staging AS SELECT * FROM official_prompt_versions WHERE 0;",
        "CREATE TABLE IF NOT EXISTS official_prompt_facets_staging AS SELECT * FROM official_prompt_facets WHERE 0;",
        "DELETE FROM official_prompts_staging;",
        "DELETE FROM official_prompt_versions_staging;",
        "DELETE FROM official_prompt_facets_staging;",
    ]
    for statement in statements:
        if statement.startswith("INSERT INTO official_prompts "):
            staged = statement.replace("INSERT INTO official_prompts ", "INSERT INTO official_prompts_staging ", 1)
            staging_statements.append(staged.split(" ON CONFLICT(id)", 1)[0] + ";")
        elif statement.startswith("INSERT OR IGNORE INTO official_prompt_versions "):
            staging_statements.append(statement.replace(
                "INSERT OR IGNORE INTO official_prompt_versions ",
                "INSERT INTO official_prompt_versions_staging ",
                1,
            ))
        elif statement.startswith("INSERT INTO official_prompt_facets "):
            staging_statements.append(statement.replace(
                "INSERT INTO official_prompt_facets ",
                "INSERT INTO official_prompt_facets_staging ",
                1,
            ))

    publish_statements = [
        f"INSERT INTO official_prompts ({prompt_columns}) SELECT {prompt_columns} FROM official_prompts_staging WHERE 1 "
        "ON CONFLICT(id) DO UPDATE SET slug=excluded.slug, current_version=excluded.current_version, title=excluded.title, "
        "short_description=excluded.short_description, user_promise=excluded.user_promise, collection=excluded.collection, "
        "category=excluded.category, template_family=excluded.template_family, default_operation=excluded.default_operation, "
        "input_mode=excluded.input_mode, output_kind=excluded.output_kind, complexity=excluded.complexity, "
        "required_input_summary=excluded.required_input_summary, preservation_summary=excluded.preservation_summary, "
        "badges=excluded.badges, aliases=excluded.aliases, search_text=excluded.search_text, "
        "thumbnail_path=excluded.thumbnail_path, thumbnail_alt=excluded.thumbnail_alt, featured_rank=excluded.featured_rank, "
        "active=1, updated_at=excluded.updated_at;",
        f"INSERT OR IGNORE INTO official_prompt_versions ({version_columns}) SELECT {version_columns} "
        "FROM official_prompt_versions_staging;",
        "DELETE FROM official_prompt_facets WHERE prompt_id IN (SELECT id FROM official_prompts_staging);",
        "INSERT INTO official_prompt_facets (prompt_id, facet_type, facet_value) "
        "SELECT prompt_id, facet_type, facet_value FROM official_prompt_facets_staging;",
        ...
        "INSERT INTO official_prompt_fts (prompt_id, title, description, user_promise, aliases, search_text) "
        "SELECT id, title, short_description, user_promise, aliases, search_text FROM official_prompts WHERE active=1;",
        next(statement for statement in statements if statement.startswith("INSERT INTO official_prompt_catalog_meta ")),
        "DROP TABLE official_prompt_facets_staging;",
        "DROP TABLE official_prompt_versions_staging;",
        "DROP TABLE official_prompts_staging;",
    ]
    statements = [*staging_statements, *publish_statements]
    header = "-- Generated by scripts/build_official_catalog.py. Do not edit.\n"
    footer = ""
    chunks: list[str] = []
    current: list[str] = []
    for statement in statements:
        single = header + statement + "\n" + footer
        if len(single.encode("utf-8")) > MAX_DRIZZLE_MIGRATION_BYTES:
            raise SystemExit("A single Sites D1 seed statement exceeds the migration size bound")
        candidate = header + "\n".join([*current, statement]) + "\n" + footer
        if len(candidate.encode("utf-8")) > MAX_DRIZZLE_MIGRATION_BYTES and current:
    ...
    if current:
        chunks.append(header + "\n".join(current) + "\n" + footer)
    if not chunks or any(len(chunk.encode("utf-8")) > MAX_DRIZZLE_MIGRATION_BYTES for chunk in chunks):
        raise SystemExit("Sites D1 seed chunking failed its size bound")
    return chunks


def drizzle_seed_paths(build_hash: str, chunk_count: int, check: bool) -> list[Path]:
    hash_prefix = build_hash.removeprefix("sha256:")[:16]
    migrations: list[tuple[int, str, int | None, Path]] = []
    if DRIZZLE_DIR.exists():
        for path in DRIZZLE_DIR.glob("*_seed_*.sql"):
            match = re.fullmatch(r"(\d{4})_seed_([0-9a-f]{16})(?:_part(\d{2}))?\.sql", path.name)
            if match:
                migrations.append((int(match.group(1)), match.group(2), int(match.group(3)) if match.group(3) else None, path))
    matching = sorted(
        [(number, part, path) for number, digest_prefix, part, path in migrations if digest_prefix == hash_prefix],
        key=lambda item: item[0],
    )
    if matching:
        paths = [path for _, _, path in matching]
        if len(paths) != chunk_count or [part for _, part, _ in matching] != list(range(1, chunk_count + 1)):
            if check:
                raise SystemExit("Sites D1 catalog seed migration chunks are stale")
            first_number = matching[0][0]
            for path in paths:
                path.unlink()
            return [DRIZZLE_DIR / f"{first_number + index:04d}_seed_{hash_prefix}_part{index + 1:02d}.sql"
                    for index in range(chunk_count)]
        return paths
    if check:
        raise SystemExit("Sites D1 catalog seed migration is stale")
    numbered_files = [int(match.group(1)) for path in DRIZZLE_DIR.glob("*.sql")
                      if (match := re.match(r"(\d{4})_", path.name))]
    next_number = max(numbered_files + [1]) + 1
    return [DRIZZLE_DIR / f"{next_number + index:04d}_seed_{hash_prefix}_part{index + 1:02d}.sql"
            for index in range(chunk_count)]
...
def main() -> None:
    ...
    drizzle_chunks = split_drizzle_seed(sql_output)
    migration_output = (ROOT / "migrations" / "0001_official_prompt_library.sql").read_text(encoding="utf-8")
    drizzle_seeds = drizzle_seed_paths(build_hash, len(drizzle_chunks), args.check)
    validate_seed(records, [sql_output])
    validate_seed(records, drizzle_chunks)
    validate_staged_deployment(sql_output, drizzle_chunks)
    ...
    if args.check:
        ...
        if any(not path.exists() or path.read_text(encoding="utf-8") != chunk
               for path, chunk in zip(drizzle_seeds, drizzle_chunks, strict=True)):
            raise SystemExit("Sites D1 catalog seed migration is stale")
    else:
