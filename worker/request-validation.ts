...
const FACET_KEYS = [
  'intents', 'inputModes', 'subjectKinds', 'outputKinds', 'preservationNeeds', 'collections', 'capabilities',
] as const
...
function stringList(value: unknown, name: string): string[] | undefined {
  ...
  if (!Array.isArray(value) || value.length > 20 || value.some((item) => typeof item !== 'string' || item.length > 100)) {
    throw new Error(`${name} must be an array of at most 20 short strings.`)
  }
  return value
}
...
export function parseOfficialPromptSearch(value: unknown): OfficialPromptSearchInput {
  ...
  if (Object.keys(record).some((key) => !allowed.has(key))) throw new Error('The request body contains unsupported fields.')
  if (typeof record.query !== 'string' || record.query.length > 300) throw new Error('query must be a string of at most 300 characters.')
